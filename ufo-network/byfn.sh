...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"ufochannel\")"
  ...
  echo
  echo "	byfn.sh generate -c ufochannel"
  echo "	byfn.sh up -c ufochannel -s couchdb"
  echo "        byfn.sh up -c ufochannel -s couchdb -i 1.4.0"
  echo "	byfn.sh up -l node"
  echo "	byfn.sh down -c ufochannel"
  echo "        byfn.sh upgrade -c ufochannel"
  echo
...
# Versions of fabric known not to work with this release of ufo-network
...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/sales.ufo.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/customer.ufo.com/ca && ls *_sk)
  fi
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.ufo.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.ufo.com
    docker cp -a orderer.ufo.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.ufo.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.ufo.com

    for PEER in peer0.sales.ufo.com peer1.sales.ufo.com peer0.customer.ufo.com peer1.customer.ufo.com; do
...
function networkDown() {
  # stop org3 containers also in addition to SalesOrg and CustomerOrg
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/ufo-network --rm hyperledger/fabric-tools:$IMAGETAG rm -Rf /tmp/ufo-network/ledgers-backup
...
# and two Peer Orgs (``SalesOrg`` & ``CustomerOrg``) each managing and maintaining two peer nodes.
...
# (``peer0.sales.ufo.com`` & ``peer0.customer.ufo.com``).  Secondly, we point to
...
# channel name defaults to "ufochannel"
